...
public partial class Sonic3DBlast
{
    ...
    {
        public ShoveEast(Sonic3DBlast pack) : base(pack) { } // bottom right
        ...
        public override bool StartAction()
        {
            if (EffectPack.rom_type == ROMType.DIRECTORS_CUT)
            {
                if (EffectPack.IsSpecialStage())
                    return Connector.Write16(DirectorsCutAddresses.ADDR_SPECIAL_STAGE_VELOCITY_X, 0x0064);
                else
            ...
            {
                if (EffectPack.IsSpecialStage())
                    return Connector.Write16(Sonic3DBlastAddresses.ADDR_SPECIAL_STAGE_VELOCITY_X, 0x0064);
                else
